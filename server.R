...
#Reactive df met SPSS data. 
  ...
  spss_data <- reactive({
    ...
    #o.b.v. verzoek om ook platte data uit te kunnen draaien; Zelfde trucje met crossings;  
    df$dummy_crossing <- 1
...
#### Stap 3 Variabelen en Crossings ####
  ...
  #zodat selecties niet verdwijnen bij het terugwisselen naar 'wel crossings'
  opgeslagen_crossings <- reactiveVal()
  
  observeEvent(input$geen_crossings,{
...
#### Stap 7: Configuratie lezen ####
  ...
  observeEvent(input$maak_kubusdata,{
    ...
    #Proberen om parameter geen_crossings op te halen. Als het niet lukt: FALSE
    #Optie om zonder crossings te draaien is nieuw. Er zijn nu veel configuraties
    #in gebruik zonder deze parameter. Op deze manier crasht de applicatie niet
    #bij oude configuraties & wordt standaard FALSE meegegeven
    geen_crossings <- try(configuratie_algemeen()$geen_crossings, silent = T)
